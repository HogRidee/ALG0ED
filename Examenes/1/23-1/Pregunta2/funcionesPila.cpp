...
void apilar(struct Pila & pila, struct Evento elemento){
    insertarAlInicio(pila.lista, elemento);
}

/* pop, elimina un elemento de la parte superior de la pila*/
struct Evento desapilar(struct Pila & pila){
	...
	return {-1,-1,-1}; //O cualquier otro valor que no se encuentre en la pila
        ...
        struct Evento elemento = cima(pila);
...
struct Evento cima(const struct Pila & pila){
