...
void insertarAlInicio(struct Lista & lista, struct Pedido elemento) {
...
void insertarAlFinal(struct Lista & lista, struct Pedido elemento) {
...
struct Nodo * crearNodo(struct Pedido elemento, struct Nodo * siguiente) {
    struct Nodo * nuevoNodo = new struct Nodo;
    nuevoNodo->elemento = elemento;
...
void imprime(const struct Lista & lista) {
    ...
    if (esListaVacia(lista)) {
        ...
        while (recorrido != nullptr) {
            ...
            cout << recorrido->elemento.id << " " << recorrido->elemento.tiempoEstimadoViaje
