...
void apilar(struct Pila & pila, struct Efectivo elemento){
    insertarAlInicio(pila.lista, elemento);
}

/* pop, elimina un elemento de la parte superior de la pila*/
struct Efectivo desapilar(struct Pila & pila){
    
    if (esPilaVacia(pila)){
	return {-1,-1,-1}; //O cualquier otro valor que no se encuentre en la pila
    }	
    else{
        struct Efectivo elemento = cima(pila);
...
struct Efectivo cima(const struct Pila & pila){
