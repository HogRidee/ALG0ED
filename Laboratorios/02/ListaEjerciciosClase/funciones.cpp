...
void imprimeRecursivo(const struct Lista & lista) {
    ...
    if (not esListaVacia(lista)) {
        recorreRecursivo(lista.cabeza);
    }
    cout << endl;
}

void recorreRecursivo(struct Nodo * recorrido) {
...
void busquedaRecursivo(const struct Lista & lista, int elemento) {
    cout << "Búsqueda recursivo... " << endl;
    if (not esListaVacia(lista)) {
        seEncuentraRecursivo(lista.cabeza, elemento);
    }
    cout << endl;
}

void seEncuentraRecursivo(struct Nodo * recorrido, int elemento) {
...
void invierteListaRecursivo(struct Lista & lista) {
    ...
    invierteLaListaRecursivo(lista.cabeza);
}

void invierteLaListaRecursivo(struct Nodo *& inicio) {
    if ((inicio == nullptr) or (inicio->siguiente == nullptr)) {
        return;
    }

    struct Nodo * recorrido = inicio->siguiente;
    ...
    inicio->siguiente = nullptr; // El nuevo último nodo debe apuntar a nullptr
...
/*método que ordena intercambiando elementos, para datos simples funciona
...
void bubbleSort(struct Lista & lista) {
    ...
    do {
        ...
        /*Ojo que compara con recorrido siguiente para dejar el último ordenado*/
        while (recorrido->siguiente != ultimo) {
            if (recorrido->elemento > recorrido->siguiente->elemento) {
                ...
                recorrido->siguiente->elemento = temp;
                intercambio = true;
            }
            recorrido = recorrido->siguiente;
        }
