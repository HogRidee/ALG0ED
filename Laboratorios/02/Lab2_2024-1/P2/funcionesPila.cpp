...
void apilar(struct Pila & pila, struct Producto elemento){
    ...
    //cout<<"Apilando: "<<elemento.peso<<endl;
}
...
struct Producto desapilar(struct Pila & pila){
	...
	return {-1,-1}; //O cualquier otro valor que no se encuentre en la pila
    }	
    else{
        struct Producto elemento = cima(pila);
...
struct Producto cima(const struct Pila & pila){
