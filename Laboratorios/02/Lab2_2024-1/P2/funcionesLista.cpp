...
void insertarAlInicio(struct Lista & tad, struct Producto elemento) {
...
void insertarAlFinal(struct Lista & tad, struct Producto elemento) {
...
struct Nodo * crearNodo(struct Producto elemento, struct Nodo * siguiente) {
    struct Nodo * nuevoNodo = new struct Nodo;
    nuevoNodo->elemento = elemento;
...
void insertarEnOrden(struct Lista & tad, struct Producto elemento) {
...
struct Nodo * obtenerNodoAnterior(const struct Lista & tad, struct Producto elemento) {
    struct Nodo * anterior = nullptr;
    struct Nodo * recorrido = tad.cabeza;
    while ((recorrido != nullptr) and (recorrido->elemento.peso < elemento.peso)) {
        anterior = recorrido;
        recorrido = recorrido->siguiente;
    }
    return anterior;
...
struct Nodo * seEncuentra(const struct Lista & tad, struct Producto elemento) {
    struct Nodo * recorrido = tad.cabeza;
...
struct Producto retornaCabeza(const struct Lista & tad){
    return tad.cabeza->elemento;
}


void eliminaNodo(struct Lista & tad, struct Producto elemento) {
    struct Nodo * ultimo = nullptr;
    struct Nodo * recorrido = tad.cabeza;
    /*Avanzo hasta encontrar el elemento*/
    /*Si no lo encuentra no elimina nada*/
    while ((recorrido != nullptr) and (recorrido->elemento.peso != elemento.peso)
            and (recorrido->elemento.calidad != elemento.calidad)) {
        ultimo = recorrido;
        recorrido = recorrido->siguiente;
    }
