...
void insertarAlFinal(struct Lista & lista, int elemento, int sucursal) {

    struct Nodo * ultimoNodo = obtenerUltimoNodo(lista);
    struct Nodo * nuevoNodo = crearNodo(elemento, nullptr, sucursal);
...
struct Nodo * crearNodo(int elemento, struct Nodo * siguiente, int sucursal) {
    struct Nodo * nuevoNodo = new struct Nodo;
    nuevoNodo->elemento = elemento;
    nuevoNodo->siguiente = siguiente;
    nuevoNodo->sucursal = sucursal;
    return nuevoNodo;
}
...
void insertarEnOrden(struct Lista & lista, int elemento) {
    ...
    struct Nodo * nuevoNodo = crearNodo(elemento, nullptr, 0);
...
void fusion(struct Lista &sabado, struct Lista &dia){
    if(esListaVacia(sabado))
        copiarLista(sabado, dia);
    else{
        struct Nodo * recorrido = dia.cabeza;
        while (recorrido != nullptr){
            colocarDato(sabado, recorrido);
            recorrido = recorrido->siguiente;
        }
    }
}
...
void copiarLista(struct Lista &sabado, struct Lista &dia){
    ...
    while (recorrido != nullptr) {
        ultimo = recorrido;
        insertarAlFinal(sabado, recorrido->elemento, recorrido->sucursal);
        recorrido = recorrido->siguiente;
    }
}

void colocarDato(struct Lista &sabado, struct Nodo *nodo) {
    struct Nodo* nuevoNodo = crearNodo(nodo->elemento, nullptr, nodo->sucursal);
    // Si la lista está vacía o el nuevo nodo debe ir al principio
    if (sabado.cabeza == nullptr || sabado.cabeza->elemento > nuevoNodo->elemento) {
        nuevoNodo->siguiente = sabado.cabeza;
        sabado.cabeza = nuevoNodo;
        return;
    }
    struct Nodo* recorrido = sabado.cabeza;
    // Recorrer la lista para encontrar la posición correcta
    while (recorrido->siguiente != nullptr && recorrido->siguiente->elemento <= nuevoNodo->elemento) {
        recorrido = recorrido->siguiente;
    }
