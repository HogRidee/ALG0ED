...
void construir(struct Lista & lista) {
    ...
    lista.cola = nullptr;
    lista.colaChibolos = nullptr;
    lista.colaViejos = nullptr;
    lista.longitud = 0;
}
...
void insertarAlInicio(struct Lista & lista, struct Paciente elemento) {
...
void insertarAlFinal(struct Lista & lista, struct Paciente elemento) {
...
//void insertarEnOrden(struct Lista & lista, struct Paciente elemento) {
//
...
//struct Nodo * seEncuentra(const struct Lista & lista, struct Paciente elemento) {
//    struct Nodo * recorrido = lista.cabeza;
//
...
//void eliminaNodo(struct Lista & lista, struct Paciente elemento) {
//    struct Nodo * ultimo = nullptr;
//    struct Nodo * recorrido = lista.cabeza;
...
void imprime(const struct Lista & lista) {
    ...
    if (esListaVacia(lista)) {
        ...
        while (recorrido != nullptr) {
            ...
            fecha = recorrido->elemento.fecha;
            aaaa = fecha/10000;
            fecha = fecha%10000;
            mm = fecha/100;
            dd = fecha%100;
...
void insertarConPrioridad(struct Lista &lista, struct Paciente paciente){
    ...
    int edad = 20240000 - paciente.fecha;
    if(esListaVacia(lista)){
        lista.cabeza = nuevoNodo;
        lista.cola = nuevoNodo;
        if(edad > 800000) lista.colaViejos = nuevoNodo;
        if(edad < 100000) lista.colaChibolos = nuevoNodo;
    }
    else{
        if(edad > 800000){
            if(lista.colaViejos != nullptr){
                nuevoNodo->siguiente = lista.colaViejos->siguiente;
                lista.colaViejos->siguiente = nuevoNodo;
                lista.colaViejos = nuevoNodo;
                if(lista.colaViejos->siguiente == nullptr) lista.cola = nuevoNodo;
            }
            else{
                nuevoNodo->siguiente = lista.cabeza;
                lista.cabeza = nuevoNodo;
            ...
            if(edad < 100000){
                if(lista.colaChibolos != nullptr){
                    nuevoNodo->siguiente = lista.colaChibolos->siguiente;
                    lista.colaChibolos->siguiente = nuevoNodo;
                    lista.colaChibolos = nuevoNodo;
                    if(lista.colaChibolos->siguiente == nullptr) lista.cola = nuevoNodo;
                }
                else{
                    if (lista.colaViejos != nullptr) {
                        nuevoNodo->siguiente = lista.colaViejos->siguiente;
                    ...
                    lista.colaChibolos = nuevoNodo;
                    if(lista.cola == lista.colaViejos) lista.cola = nuevoNodo;
                }
            }
            else{
