...
//void insertarEnOrden(struct Lista & lista, struct Evento elemento) {
//
...
//struct Nodo * obtenerNodoAnterior(const struct Lista & lista, struct Evento elemento) {
...
//struct Nodo * seEncuentra(const struct Lista & lista, struct Evento elemento) {
//    struct Nodo * recorrido = lista.cabeza;
//
...
//void eliminaNodo(struct Lista & lista, struct Evento elemento) {
//    struct Nodo * ultimo = nullptr;
//    struct Nodo * recorrido = lista.cabeza;
...
void insertarConPrioridad(struct Lista &lista, struct Evento evento){
    struct Nodo * nuevoNodo = crearNodo(evento, nullptr);
    if(esListaVacia(lista)){
        lista.cabeza = nuevoNodo;
        lista.cola = nuevoNodo;
        if(evento.prioridad == 3) lista.cola3 = nuevoNodo;
        if(evento.prioridad == 2) lista.cola2 = nuevoNodo;
    }
    else{
        if(evento.prioridad == 3){
            if(lista.cola3 != nullptr){
                nuevoNodo->siguiente = lista.cola3->siguiente;
                lista.cola3->siguiente = nuevoNodo;
                lista.cola3 = nuevoNodo;
                if(lista.cabeza == lista.cola) lista.cola = nuevoNodo;
            }
            else{
                lista.cabeza = nuevoNodo;
                lista.cola3 = nuevoNodo;
                if(lista.cola2 == nullptr and lista.cola != nullptr){
            ...
            if(evento.prioridad == 2){
                if(lista.cola2 != nullptr){
                    nuevoNodo->siguiente = lista.cola2->siguiente;
                    lista.cola2->siguiente = nuevoNodo;
                    lista.cola2 = nuevoNodo;
                }
                else{
                    lista.cola2 = nuevoNodo;
                    nuevoNodo->siguiente = lista.cola3->siguiente;
